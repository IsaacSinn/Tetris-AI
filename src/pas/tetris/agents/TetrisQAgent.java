    ...
    extends QAgent
{
                                    ...
                                    final Mino potentialAction)
    {
        ...
        for (int i = 0; i < cols; i ++) {
            ...
            for (int j = 0; j < rows; j++) {
                ...
                if (grayScale.get(j,i) != 0.0) {
                    ...
                    int height = j;
                    heightsHoles.set(0, i, j);
                    maxHeight = Math.max(maxHeight, rows - height);
                    found = true;
            ...
            if (!found) {
                heightsHoles.set(0, i, rows);
            }
        }

        if (maxHeight == 0) {
            return Matrix.zeros(1, 37);
        }
        ...
        for (int i = 0; i < cols; i++) {
            for (int j = 22 - maxHeight; j < rows; j++) {
                if (grayScale.get(j, i) == 0.0) {
                    totalHoles++;
                }
            }
    ...
    public double getReward(final GameView game)
    {
        ...
        */
        ...
        currentReward = -0.51 * height + 0.76 * lines - 0.36 * holes - 0.18 * bumpiness + 7 * game.getScoreThisTurn();
