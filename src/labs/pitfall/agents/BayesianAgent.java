    ...
    extends Agent
{
        ...
        extends Object
    {
        ...
        {
            // create a list of probabilities for each frontier coordinate
            List<Double> probs = new ArrayList<Double>();

            // get all frontier coordinates and store in an array
            List<Coordinate> frontierPitCoordinates = new ArrayList<Coordinate>();
            
            for(Coordinate coord : this.frontierPitCoordinates)
            {
                probs.add(0.0);
                frontierPitCoordinates.add(coord);
            }

            // for each frontier coordinate, create a list of possible pit configurations. For example: assume first coordinate is a pit, generate [1, 0, 0, 0], [1, 1, 0, 0] ... [1, 1, 1, 1] etc.
            for (int i = 0; i < frontierPitCoordinates.size(); i++)
            {
                Coordinate coord = frontierPitCoordinates.get(i);
                List<List<Boolean>> allPitConfigs = new ArrayList<List<Boolean>>();
                
                // generate all possible pit configurations
                for (int j = 0; j < Math.pow(2, this.frontierPitCoordinates.size()); j++)
                {
                    List<Boolean> pitConfig = new ArrayList<Boolean>();
                    for (int k = 0; k < this.frontierPitCoordinates.size(); k++)
                    {
                        if (k == i)
                        {
                            pitConfig.add(true);
                        ...
                        {
                            pitConfig.add((j & (1 << k)) != 0);
                        }
                    }
                    allPitConfigs.add(pitConfig);
                }
                System.out.println(allPitConfigs);
                break;
            }

            return frontierPitCoordinates.get(0);
