...
public class BayesianAgentEasy
    extends Agent
{
        ...
        extends Object
    {
        ...
        public Coordinate getNextCoordinateToExplore()
        {
            ...
            System.out.println("Frontier Coordinates: " + this.getFrontierPitCoordinates());
            System.out.println("Breeze Coordinates: " + this.getKnownBreezeCoordinates());

            // put 1 in all of the frontier coordinates
            for (Coordinate frontierCoord : this.getFrontierPitCoordinates()) {
                probabilities.put(frontierCoord, 1.0);
                totalProb += 1.0;
            }

            // Calculate Pr[Pit_X = true && evidence] for each frontier coordinate
            for (Coordinate frontierCoord : this.getFrontierPitCoordinates()) {
                double prob = calculateProbabilityOfPitGivenEvidence(frontierCoord);

                // update the probability of the coordinate containing a pit
                prob += probabilities.get(frontierCoord);
                probabilities.put(frontierCoord, prob);
                totalProb += prob;
            }


            // Normalize the probabilities to get Pr[Pit_X = true | evidence]
            for (Coordinate coord : probabilities.keySet()) {
                probabilities.put(coord, probabilities.get(coord) / totalProb);
            }

            System.out.println("Probabilities: " + probabilities);

            // Find the coordinate with the lowest probability of having a pit
            ...
            // print chose
            System.out.println("Chose: " + toExplore);

            return toExplore;
        }

        // Check if two coordinates are next to each other
        private boolean isAdjacent(Coordinate coord, Coordinate other) {
            int xDiff = Math.abs(coord.getXCoordinate() - other.getXCoordinate());
            int yDiff = Math.abs(coord.getYCoordinate() - other.getYCoordinate());

            // One coordinate difference in either x or y direction, but not both
            return (xDiff == 1 && yDiff == 0) || (xDiff == 0 && yDiff == 1);
        }

        private double calculateProbabilityOfPitGivenEvidence(Coordinate coord) {
            // loop through all the known breeze coordinates and if coor is adjacent to a known breeze coordinate then increse the probability

            double prob = 0;
            
            for (Coordinate breezeCoord : this.getKnownBreezeCoordinates().keySet()) {
                if (isAdjacent(coord, breezeCoord) && this.getKnownBreezeCoordinates().get(breezeCoord)) {
                    System.out.println("Adjacent: " + coord + " " + breezeCoord);
	...
	public BayesianAgentEasy(int playerNum, String[] args)
	{
        super(playerNum);
